...
function App() {
  ...
  const quitarSuma = async (id) => {
    if (confirm("¿Desea quitar suma?")) {
      const response = await fetch(`http://localhost:3000/${operacion}/${id}`, {method: "DELETE",});
  ...
  return (
    <>
      ...
      <ul>{sumas.map((suma) => (
        <li key={suma.id}>{`${suma.id}: ${suma.a} ${signo} ${suma.b} = ${suma.resultado} `}
            <button onClick={() => modificarSuma(suma)} disabled={sumaId !== 0}>E</button>
            <button onClick={() => quitarSuma(suma.id)} disabled={sumaId !== 0}>X</button>
          </li>
        ))}
      </ul>
    </>
