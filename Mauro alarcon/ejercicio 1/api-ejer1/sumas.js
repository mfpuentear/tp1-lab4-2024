import express from "express";
import { body, validationResult } from "express-validator";

export const sumasRoute = express.Router();

let sumas = [
  { id: 1, a: 8, b: 2, resultado: 10 },
  { id: 2, a: 18, b: 3, resultado: 21 },
  { id: 3, a: 880, b: 8, resultado: 888 },
];

let sumasMaxid = sumas.length;

// Obtener todas las sumas
sumasRoute.get("/", (req, res) => {
  return res.json({ data: sumas });
});

// Obtener una suma por ID
sumasRoute.get("/:id", (req, res) => {
  const { id } = req.params;
  const sumaEncontrada = sumas.find((sum) => sum.id === parseInt(id));

  if (!sumaEncontrada) {
    return res.status(404).json({ mensaje: "Suma no encontrada" });
  }

  return res.json({ data: sumaEncontrada });
});

// Agregar una nueva suma con validación
sumasRoute.post(
  "/",
  [
    body("a").isNumeric().withMessage("El valor 'a' debe ser un número"),
    body("b").isNumeric().withMessage("El valor 'b' debe ser un número"),
  ],
  (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errores: errors.array() });
    }

    const { a, b } = req.body;
    const nuevaSuma = { id: ++sumasMaxid, a, b, resultado: a + b };

    sumas.push(nuevaSuma);
    return res.status(201).json({ data: nuevaSuma });
  }
);

// Eliminar una suma por ID
sumasRoute.delete("/:id", (req, res) => {
  const { id } = req.params;
  const sumaEncontrada = sumas.find((sum) => sum.id === parseInt(id));

  if (!sumaEncontrada) {
    return res.status(404).json({ mensaje: "Suma no encontrada" });
  }

  sumas = sumas.filter((sum) => sum.id !== parseInt(id));
  return res.status(200).json({ mensaje: "Suma eliminada", id });
});

// Actualizar una suma existente
sumasRoute.put(
  "/:id",
  [
    body("a").isNumeric().withMessage("El valor 'a' debe ser un número"),
    body("b").isNumeric().withMessage("El valor 'b' debe ser un número"),
  ],
  (req, res) => {
    const { id } = req.params;
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errores: errors.array() });
    }

    const { a, b } = req.body;
    const sumaActualizada = {
      id: parseInt(id),
      a,
      b,
      resultado: a + b,
      fecha: new Date(),
    };

    sumas = sumas.map((sum) => (sum.id === sumaActualizada.id ? sumaActualizada : sum));

    return res.status(200).json({ data: sumaActualizada });
  }
);