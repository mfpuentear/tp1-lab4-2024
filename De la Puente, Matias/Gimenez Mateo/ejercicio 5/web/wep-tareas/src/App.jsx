...
function App() {
  ...
  const [contadorcompletadas , setcontadorcompletadas] = useState(0)
  const [contadorNocompletadas , setcontadorNocompletadas] = useState(0)
  ...
  const contador = (tareas) => {
    ...
    setcontadorcompletadas(TareasCompletadas)

    const TareasNoCompletadas = tareas.reduce((acc,tarea)=> acc + (tarea.completada ? 0 : 1),0)
    setcontadorNocompletadas(TareasNoCompletadas)
  }
  ...
  return (
    <>
      <div style={{textAlign:"center"}}>
        ...
        <p>{`Tareas completadas: ${contadorcompletadas}`}</p>
        <p>{`Tareas NO completadas: ${contadorNocompletadas}`}</p>
      </div>
    </>
  )
}
