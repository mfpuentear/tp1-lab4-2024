    ...
    <>
      ...
      <ul>
        {calculos.map((calculo) => (
          <li key={calculo.id}>
            {calculo.id} Base: {calculo.base} - Altura: {calculo.altura} - Perimetro:{" "}
            {calculo.perimetro} - Area: {calculo.area} - Tipo:{" "}
            {calculo.base === calculo.altura ? "Cuadrado" : "Rectangulo"} -{" "}
            <button onClick={() => editarCalculo(calculo)}>Editar</button>
            <button onClick={() => eliminarCalculo(calculo.id)}>Eliminar</button>
          </li>
        ))}
      </ul>
    </>
